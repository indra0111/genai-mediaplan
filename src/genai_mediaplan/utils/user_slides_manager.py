...
def update_slides_content(user_id: str, presentation_id: str, data: dict):
    """Update slides content using user-specific services"""
    services = auth_manager.get_user_services(user_id)
...
def replace_table_cell_text(user_id: str, table_element, table_object_id, row_index, col_index, new_text):
    """Replace table cell text using user-specific services"""
    services = auth_manager.get_user_services(user_id)
    slides_service = services['slides']
...
def update_requests_for_tablular_data_in_slides(user_id: str, presentation_id: str, data_rows, table_alt_text):
    """Update tabular data in slides using user-specific services"""
    services = auth_manager.get_user_services(user_id)
    slides_service = services['slides']
            ...
            if element.get('description') == table_alt_text and 'table' in element:
                ...
                for row in range(len(data_rows)):
                    for col in range(len(data_rows[row])):
                        requests.extend(replace_table_cell_text(user_id, table, object_id, row+1, col, data_rows[row][col]))

    return requests

def get_tabular_data_for_forecast_tables(preset_name, audience_forecast):
    """Get tabular data for forecast tables"""
...
def get_update_requests_for_numerical_data_in_slides(user_id: str, presentation_id: str, audience_forecast):
    """Get update requests for numerical data in slides"""
    final_requests = []
    preset_alt_mapping={
    ...
    for preset in ["TIL_All_Cluster_RNF", "TIL_All_Languages_RNF", "TIL_TOI_Only_RNF", "TIL_ET_Only_RNF", "TIL_ET_And_TOI_RNF", "TIL_NBT_Only_RNF"]:
        ...
        country_tier_state_requests = update_requests_for_tablular_data_in_slides(user_id, presentation_id, country_tier_state_data, f"{preset_alt_mapping[preset]}_country_tier_state")
        city_requests = update_requests_for_tablular_data_in_slides(user_id, presentation_id, city_data, f"{preset_alt_mapping[preset]}_city")
        final_requests.extend(country_tier_state_requests + city_requests)
    return final_requests

def delete_slides_requests(user_id: str, presentation_id: str, persona_slide_index):
    """Delete slides requests using user-specific services"""
    services = auth_manager.get_user_services(user_id)
    slides_service = services['slides']
...
def get_persona_slide_index(data):
    """Get persona slide index based on data"""
    desired_count = 6
    for i in range(6):
...
def create_presentation_for_user(user_id: str, cohort_name: str, llm_response_json, audience_forecast):
    """Create a new presentation for a user in their Google Drive"""
    services = auth_manager.get_user_services(user_id)
    drive_service = services['drive']
    slides_service = services['slides']
    sheets_service = services['sheets']
    
    # Copy the source presentation to user's drive
    copied_file = drive_service.files().copy(
        fileId=SOURCE_FILE_ID,
        body={
            'name': f'TIL_CohortDashboard_{"".join(cohort_name.split(" "))}_Forecast_{datetime.now().strftime("%B%Y")}',
            'parents': []  # This will save to user's root drive
        }
    ).execute()
    
    # Copy the source sheet to user's drive
    ...
    delete_requests = delete_slides_requests(user_id, copied_file_id, persona_slide_index_to_discard)
    
    # Update slide content
    update_requests = update_slides_content(user_id, copied_file_id, data)
    
    # Update numerical data in tables
    update_requests_numerical = get_update_requests_for_numerical_data_in_slides(user_id, copied_file_id, audience_forecast)
...
def update_user_presentation(user_id: str, presentation_id: str, cohort_name: str, llm_response_json, audience_forecast):
    """Update an existing presentation for a user"""
    services = auth_manager.get_user_services(user_id)
    slides_service = services['slides']
    sheets_service = services['sheets']
    ...
    # Update persona content
    update_persona_content(slides_service, presentation_id, persona_slide_index_to_keep, persona_data)
    
    # Delete unnecessary slides
    delete_requests = delete_slides_requests(user_id, presentation_id, persona_slide_index_to_discard)
    
    # Update slide content
    update_requests = update_slides_content(user_id, presentation_id, data)
    
    # Update numerical data in tables
    update_requests_numerical = get_update_requests_for_numerical_data_in_slides(user_id, presentation_id, audience_forecast)
    
    # Update charts
    update_charts_in_slides(presentation_id, slides_service, SOURCE_SHEET_ID, sheets_service, CHART_SLIDE_INDEX, None)
